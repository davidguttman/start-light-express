...
function render (state) {
    return html`
      <div class="content">
        ...
        ${state.success
          ? html`<div class="success">${state.success}</div>`
          : ''}
        ...
        ${state.loading
          ? html` <div class="loading">Loading widgets...</div> `
          : ''}
        ${state.widgets && state.widgets.length > 0
          ? html`
                  ...
                  widget => html`
                    <div class="widget-item">
                      ...
                      <div class="widget-meta">
                        ...
                        ${widget.metadata && widget.metadata.description
                          ? html`
                              <p>
                                <strong>Description:</strong> ${widget.metadata
                                  .description}
                              </p>
                            `
                          : ''}
            ...
            `
          : state.widgets && state.widgets.length === 0
          ? html` <p>No widgets found. Create your first widget above!</p> `
          : ''}
      </div>
    `
}
    ...
    try {
      ...
      if (error.message !== 'Session expired') {
        state.set({
          error:
            error.message ||
            'Failed to load widgets. Please check if the server is running.',
          loading: false
        })
      }
    }
...
async function deleteWidget (id) {
    ...
    try {
      ...
      if (error.message !== 'Session expired') {
        state.set({
          error: error.message || 'Failed to delete widget. Please try again.'
        })
      }
    }
}
